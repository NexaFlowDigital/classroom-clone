...
widgetSelect.onchange = e => {
  ...
  if (type && widgetRegistry[type]) {
    try {
      const result = widgetRegistry[type]();
      if (result?.w) {
        result.w.style.zIndex = ++topZ;
        saveCurrentScreen();
      }
    } catch (err) {
      console.error('Widget error:', err);
      alert('Failed to load widget: ' + type);
    }
  }
  widgetSelect.value = '';
};
